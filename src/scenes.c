...
const uint8_t * __tiles;
static const uint8_t * __empty;
...
void redraw_scene(scene_item_t * scene) NAKED PRESERVES_REGS(b, c) {
    scene;
__asm
        push    BC
...
1$:
        ...
        add     HL, DE
        ld      C, L
        ld      B, H            ; BC: spr = (uint8_t *)&__tiles[(int)__put_map_id << 7u]
...
void erase_item(scene_item_t * item) NAKED PRESERVES_REGS(b, c) {
    item;
__asm
        push    BC
...
static uint8_t __put_map_id;
void put_map() {
    static uint8_t i, oy, dy;
    static uint8_t * data1, * data2, * spr, * mask, * limit;

    if (__put_map_y < ((viewport_height - 1) * 8)) {
        oy = __put_map_y >> 3u;

        if (__put_map_id != 0xffu) {
            spr = (uint8_t *)&__tiles[(int)__put_map_id << 7u];
            mask = spr + 0x40u;
        }  else {
            spr = mask = (uint8_t *)__empty;
        }

        dy = (__put_map_y & 7u);

        data1 = (uint8_t *)shadow_rows[oy];
...
void clear_map(scene_t * dest) NAKED PRESERVES_REGS(b, c) {
    dest;
__asm
        ld      H, D
        ld      L, E
...
void clear_map(scene_t * dest) {
    static uint8_t * tmp;
    static uint16_t sz;
    sz = sizeof(*dest), tmp = (uint8_t *)dest;
    while (sz) *tmp++ = 0u, sz--;
}
*/
